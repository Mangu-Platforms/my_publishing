...
| **2d.1** | **Pages & Components**      | Progressive dual-run via `lib/data/*` (default Supabase; Mongo when `DATABASE_PROVIDER=mongodb`). **Slice A:** home Featured/Trending/GenreExplorer/Stats + `/genres` + `/genres/[genre]` + `/checkout` book load. **Slice B:** `/books` browse `q`/`sort` parity + Author Spotlight via `lib/data/authors.ts`. **Remaining:** book detail leftovers, library/reading/entitlements, admin/dashboard portals. Doc paths `app/(shop)/…` do not exist — live tree is `app/(consumer)/…` (A-16). | `lib/data/*`, `app/(consumer)/**/*.tsx`, `components/home/*`, `app/checkout/page.tsx` | `tsc --noEmit` clean; dual-run unit tests; default provider still Supabase     |
