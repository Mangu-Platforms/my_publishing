...
| **2d.1** | **Pages & Components**      | Progressive dual-run via `lib/data/*` (default Supabase; Mongo when `DATABASE_PROVIDER=mongodb`). **Landed (PR #349):** consumer catalog/PDP/reading/library/authors/audio/recs; PDP + GET `/api/reviews`; admin dashboard/books/orders/users reads; author portal dashboard; my-reviews. See `docs/PHOENIX_WS2D_AGENT_HANDOFF.md`. **Remaining:** admin **writes** (`admin/actions`), manuscripts edit/new, partner portal, engagement APIs. Live tree `app/(consumer)/…` (A-16). | `lib/data/*`, `lib/reading/entitlement.ts`, `app/**/*.tsx` | `tsc --noEmit` clean; dual-run unit tests; default provider still Supabase; handoff doc current |
