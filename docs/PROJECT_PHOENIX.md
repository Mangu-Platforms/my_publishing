...
**Document Version:** 4.0.3 (WS2d Catalog Dual-Run Slice)  
...
| **2d.1** | **Pages & Components**      | Progressive dual-run via `lib/data/*` (default Supabase; Mongo when `DATABASE_PROVIDER=mongodb`). **Slices A–E:** home rails, genres, checkout, `/books` q/sort, Author Spotlight, enriched PDP `fetchBookForApi`, dual-run entitlement (A-6), `/reading` + `/library`, author PDP, recommendations baseline, PDP reviews via `lib/data/reviews.ts`. **Remaining:** admin/dashboard portals, engagement APIs, GET `/api/reviews` mongo path. Doc paths `app/(shop)/…` do not exist — live tree is `app/(consumer)/…` (A-16). | `lib/data/*`, `lib/reading/entitlement.ts`, `app/(consumer)/**/*.tsx`, `components/home/*` | `tsc --noEmit` clean; dual-run + entitlement + reviews unit tests; default provider still Supabase |
...
| A-16 | §5 WS2d    | Task 2d.1 cited `app/(shop)/books/page.tsx` and `components/BookCard.tsx`; live App Router uses `app/(consumer)/…` and `components/cards/BookCard.tsx` | 2d.1 rewritten as progressive dual-run slices through `lib/data/*`; Slice A = home/genres/checkout                         |
