...
| **2d.1** | **Pages & Components**      | Progressive dual-run via `lib/data/*` (default Supabase; Mongo when `DATABASE_PROVIDER=mongodb`). **Slice A (landed):** home Featured/Trending/GenreExplorer/Stats + `/genres` + `/genres/[genre]` + `/checkout` book load. **Remaining:** `/books` stream, book detail leftovers, library/reading/entitlements, author spotlight, admin/dashboard portals. Doc paths `app/(shop)/…` do not exist — live tree is `app/(consumer)/…` (A-16). | `lib/data/*`, `app/(consumer)/**/*.tsx`, `components/home/*`, `app/checkout/page.tsx` | `tsc --noEmit` clean; dual-run unit tests; default provider still Supabase     |
