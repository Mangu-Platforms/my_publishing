...
**Governance defect (not a brief conflict): two live ADR-001s.** `docs/adr/ADR-001-canonical-platform.md` and `docs/architecture/ADR-003-catalog-and-identity-data-ownership.md` share a number; the latter itself forbids bare "ADR-001" citations. This report uses full paths; recommend renumbering in PR-D1.
